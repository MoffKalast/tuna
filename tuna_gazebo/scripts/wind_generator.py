...
class WindGen:
	def __init__(self):
		rospy.init_node('gazebo_wind_generator')
		...
		rospy.loginfo("Wind Generator started")
	...
	def update(self):
		...
		self.dir_x = self.dir_x * 0.97 + (random.random()*WIND_CHANGE_SPD - WIND_CHANGE_SPD/2) * 0.03
		self.dir_y = self.dir_y * 0.97 + (random.random()*WIND_CHANGE_SPD - WIND_CHANGE_SPD/2) * 0.03

		self.vel.x = clamp(self.vel.x + self.dir_x, -MAX_WIND, MAX_WIND)
		self.vel.y = clamp(self.vel.y - self.dir_y, -MAX_WIND, MAX_WIND)
